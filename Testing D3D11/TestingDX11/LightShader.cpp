...
bool LightShader::SetShaderParameters(ID3D11DeviceContext *& deviceContext, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, DirectX::XMMATRIX projectionMatrix,
	DirectX::XMFLOAT3 lightPos, DirectX::XMFLOAT3 lightColor, float radius, float lightIntensity,PointLight light[], DirectX::XMFLOAT3 camPos)
{
...
//	D3D11_MAPPED_SUBRESOURCE mappedMemorySpec;
	...
	dataSpec->x = lightPos.x;//temp[0].x; 
	dataSpec->y = lightPos.y;//temp[0].y;
	dataSpec->z = lightPos.z;//temp[0].z;
	
	dataSpec->w = light[0].w;//temp[0].w;
	dataSpec->r = lightColor.x;//temp[0].r;
	dataSpec->g = lightColor.y;//temp[0].g;
	dataSpec->b = lightColor.z;
	dataSpec->a = lightIntensity;
	dataSpec->world = worldMatrix;
	dataSpec->cx = camPos.x;//temp[0].x; 
	dataSpec->cy = camPos.y;//temp[0].y;
	dataSpec->cz = camPos.z;//temp[0].z;
	dataSpec->cw = 10.0f;//temp[0].w;
	/*dataSpec->lightPos = lightPos;
	dataSpec->lightIntensity = lightIntensity;
	dataSpec->lightColor = lightColor;
	dataSpec->radius = radius;*/
	...
	// Get a pointer to the data in the constant buffer.
	//dataSpec = (PointLight*)mappedMemory.pData; //was in tutorial
